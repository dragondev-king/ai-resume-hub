...
/** Exact main-branch system prompt base (bullets, skills, version rules). */
const SYSTEM_PROMPT =
...
/**
 * When company/role tailoring is OFF: same quality as main, but never rename experience
 * positions (especially the latest role) to the JD title.
 */
const SYSTEM_PROMPT_KEEP_POSITION_TITLES =
  'You are an expert resume writer specializing in career transitions and role-specific tailoring. Transform the candidate\'s experience so they look like a strong fit for the target job. Write rich, specific, human bullets a recruiter would believe. Generate 7-12 bullet points per work experience (10-12 for longer or senior roles). Never write thin 4-5 bullet roles. Extract the job title and company name from the job description. Aggressively tailor bullet points, summary, and skills while keeping company names, employment dates, and EACH experience position title EXACTLY as listed in the work history. Never change the most recent role title to match the job description title. In experience bullet points, wrap each technical skill/tool/framework/language with <b>...</b>. Version rule: required job-description versions belong in the most recent company only, once per version; earlier companies and the summary use family names with no version number. Never put a version in a job that ended before that version existed.';
...
/** When company-tailor is OFF, keep profile position titles (never JD-renamed last role). */
function restoreOriginalPositionTitles(parsed: any, profile: any): any {
  if (!parsed || !Array.isArray(parsed.experience)) return parsed;
  const original = Array.isArray(profile?.experience) ? profile.experience : [];
  return {
    ...parsed,
    experience: parsed.experience.map((exp: any, index: number) => ({
      ...exp,
      position: original[index]?.position || exp.position || '',
      company: original[index]?.company || exp.company || '',
    })),
  };
}
  ...
  try {
    ...
    const draft = await generateResumeDraft({
      ...
      system: shouldTailorCompanies
        ...
        : SYSTEM_PROMPT_KEEP_POSITION_TITLES,
    ...
    // OFF = main quality, but force profile position/company titles (blocks JD-title-as-last-role)
    if (!shouldTailorCompanies) {
      const parsedCheck = restoreOriginalPositionTitles(
        normalizeParsedResume(parseJsonLoose(rawResponse)),
        profile
      );
      ...
      return res.status(200).json({
        success: true,
        aiResponse: JSON.stringify(parsedCheck),
        provider,
        companyPick: null,
      });
    }
...
2. TRANSFORM each work experience to align with the target role:
   - ${
     tailor
       ...
       : 'KEEP each experience position title EXACTLY as listed in WORK HISTORY (do not rename the latest role to the JD title)'
   }
...
${
  tailor
   ...
   - Keep company names exactly as provided in WORK HISTORY`
    : `   - Copy every experience "position" EXACTLY from WORK HISTORY
   - Do NOT change the most recent / last role title to the job description title
   - Do NOT invent Junior / Associate / Senior ladders
...

{
  ...
  "experience": [
    {
      "position": "${tailor ? 'Junior→Senior ladder title for this role' : 'Exact position title from WORK HISTORY'}",
