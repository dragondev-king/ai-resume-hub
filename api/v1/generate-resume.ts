...
import {
  ...
  resumeFunctionConfig,
  type ResumeVersionConfig,
} from '../_resumeGeneration';

export const config = resumeFunctionConfig;

const v1: ResumeVersionConfig = {
  version: 'v1',
  system:
  ...
  timelineSystem: `You map job-description technologies onto a candidate's real work history. A specific version must not appear in a job that ended before it existed. Required JD versions belong in mustUse for the MOST RECENT role only. Each version should be named once in that role's bullets, then family names only. All earlier roles: family name in mayUse, required version in mustNotUse. Respond with valid JSON only.`,
  auditSystem: `You are a light copy editor. Do not rewrite the resume. Do not shorten it. Do not drop bullets or skills. Keep the same dates, companies, bullet count, and skill list length. Only fix version-number placement. Respond with valid JSON only.`,
...

{
  ...
  buildAuditPrompt: ({ today, jobDescription, workHistory, timeline, draft }) => `TODAY'S DATE: ${today}
...
${timeline || 'Required JD versions may be named in the most recent company only, and only if that role was still active after the version shipped. Summary: family names only, no versions. Earlier jobs: family names, no version numbers.'}
...
export const resumeVersionV1 = v1;

export default async function handler(req: VercelRequest, res: VercelResponse) {
  return handleGenerateResume(req, res, v1);
}
