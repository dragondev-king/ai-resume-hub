...
-- Add the active column with default value true (all existing profiles are active by default)
ALTER TABLE profiles 
ADD COLUMN IF NOT EXISTS is_active BOOLEAN DEFAULT true;

-- Add comment for clarity
COMMENT ON COLUMN profiles.is_active IS 'Whether the profile is active (true) or inactive (false)';
...
BEGIN
  IF p_profile_id IS NULL THEN
    INSERT INTO profiles (user_id, first_name, last_name, title, email, phone, location, linkedin, portfolio, summary, experience, education, skills, resume_filename_format, is_active)
    VALUES (p_user_id, p_first_name, p_last_name, p_title, p_email, p_phone, p_location, p_linkedin, p_portfolio, p_summary, p_experience, p_education, p_skills, p_resume_filename_format, p_is_active)
...
RETURNS TABLE (
  ...
  resume_filename_format TEXT, is_active BOOLEAN, created_at TIMESTAMP WITH TIME ZONE, updated_at TIMESTAMP WITH TIME ZONE,
...
BEGIN
  ...
  RETURN QUERY SELECT 
    ...
    p.resume_filename_format, p.is_active, p.created_at, p.updated_at, 
...
SELECT 'Profile active field added successfully!' as status;
