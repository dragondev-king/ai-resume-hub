-- =====================================================
-- 17. APPLICATION SUMMARY FUNCTIONS
-- =====================================================
-- This creates functions to get application summaries for the home page dashboard

-- Function to get application summaries by profile owner
CREATE OR REPLACE FUNCTION get_application_summaries_by_owner(
  p_user_id UUID, 
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(DISTINCT p.id)::INTEGER as profile_count,
  ...
  HAVING COUNT(DISTINCT p.id) > 0  -- Only show owners who have profiles
  ORDER BY this_week_applications DESC, this_month_applications DESC, today_applications DESC, u.first_name, u.last_name;
END; $$;

-- Function to get application summaries by profile for a specific owner
CREATE OR REPLACE FUNCTION get_application_summaries_by_profile_for_owner(
  p_owner_id UUID,
  p_user_id UUID, 
  p_user_role user_role
)
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION get_application_summaries_by_owner(UUID, user_role) TO authenticated;
GRANT EXECUTE ON FUNCTION get_application_summaries_by_profile_for_owner(UUID, UUID, user_role) TO authenticated;
